...
if (!require("viridis")) install.packags("viridis")
...
if (!require("skimr")) install.package("skimr")
# Load the required library
library(skimr)
```
...
```
# Or if we play with colour within aes() or defined aesthethic. 
```
ggplot(data = specimen_data) +
  geom_point(mapping = aes(x = family_name, y = recordID, color = subfamily_name))
